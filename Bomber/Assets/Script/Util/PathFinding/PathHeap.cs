...
public class PathHeap {
	...
	private void downHeap( int idx_ )
	{
		...
		if (lChildIdx >= pathHeap.Count) { // no children
			return;
		}

		if (rChildIdx >= pathHeap.Count || heuristicCost( pathHeap[ lChildIdx ], dest ) < heuristicCost( pathHeap[ rChildIdx ], dest ) ) {		// no right children
